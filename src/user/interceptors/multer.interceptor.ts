...
import { CallHandler, ExecutionContext, mixin, NestInterceptor, Type, BadRequestException, UnprocessableEntityException, UnauthorizedException, ForbiddenException } from '@nestjs/common';
...
export function HandleImgsInterceptor(uploadFields: UploadField[]): Type<NestInterceptor> {
    class MixinInterceptor implements NestInterceptor {
        ...
        constructor() {
            console.log('a constructor here');
        }
...
async function handleMulterObjectToImgUrl(multerObject: any, appUrl: string) {
    ...
    if (!validExtension) {
        throw new BadRequestException({ message: "here is the bad request" });
    }
...
async function checkValidImgUrl(req: any, field: UploadField) {
    ...
    for (const imgUrl of fieldDataInBody) {
...
interface handleIncomingRawImgsObjectAndKeepExistingOnesOptions {
...
async function handleIncomingRawImgsObjectAndKeepExistingOnes(
    ...
    if (req.body[field.name]) {
        ...
        if (field.maxCount > 1) {
            ...
            if (options.collectionOfResizeOptions && options.collectionOfResizeOptions.length > 0) {
                const resizedFieldInBody: any = [];
                for (const imgUrl of req.body[field.name]) {
                    const resizedImgUrl = { original: imgUrl };
    ...
    if (req.files[field.name]) {
        ...
        if (field.maxCount > 1) {
            ...
            for (const imgAsMulterObject of req.files[field.name]) {
                let imgUrl: any = await handleMulterObjectToImgUrl(imgAsMulterObject, req.appUrl);

                if (options.collectionOfResizeOptions && options.collectionOfResizeOptions.length > 0) {
                    imgUrl = { original: imgUrl };

                    for (const resizeOptions of options.collectionOfResizeOptions) {
...
export const handleImgsFieldsMiddleware = (fields: UploadField[]) => {
    return async (req, res, next) => {
        ...
        console.log('hello');
        try {
            const handleImgsOperations: any = [];

            for (const field of fields) {
                // validate and handle stringified fields
                await validateAndNormalizeImg(req, field);
                console.log("TCL: handleImgsFieldsMiddleware -> field", field);

                const collectionOfResizeOptions: ResizeOptions[] = [];
...
async function resizeImg(imgAsMulterObject, resizeOptions, appUrl) {
    ...
    const resizedFileName = `${imgAsMulterObject.filename.split('.')[0]}_${extraFileNamePostfix}`;

    const fullRadius = 128,
        radius = fullRadius / 2,
